...
However, it will not work for ```sysupgrade``` and ```syspatch```, and you need to export the variable ```bash FETCH_CMD="/usr/local/bin/curl -L -s -q -N -x socks5h://127.0.0.1:9050" 
in your environment to make pkg_* programs able to use the mirror.
